...
variants = ['кот', 'шеф', 'мозг', 'лес', 'фолк', 'код', 'рот', 'мёд',
            'лук', 'год', 'час', 'друг', 'муж', 'айфон']

number_proverbs = input('Сколько пословиц Вам вывести на экран?:')
number_proverbs = int(number_proverbs)
str_proverbs = ''
for proverb in proverbs:
    ...
    for letter in variants:
        random_letter = choice(variants)
    str_proverbs += random_proverb.replace('Ум', random_letter)
    result = str_proverbs.split('.')
for i in range(number_proverbs):
    print(f'{i + 1}.{result[i].capitalize()}')
